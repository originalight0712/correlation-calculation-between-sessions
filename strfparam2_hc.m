%
%function [RFParam]=strfparam2(taxis,faxis,STRF,Wo,PP,Sound,MaxFm,MaxRD,Thresh,alpha1,alpha2,Disp)
%
...
%       .PeakDelayP : Temporal delay at STRF Possitive Peak(msec) 
%       .PeakBFP    : Best frequency at STRF Possitive Peak (Octaves)
%       .PeakDelayN : Temporal delay at STRF Negative Peak(msec) 
%       .PeakBFN    : Best frequency at STRF Negative Peak (Octaves)
...
%                                     April 2021 - added negative and positive peak delays)


%adding RTF to return the value
function [Fm,RD,RTF,TFParam,RFParam]=strfparam2_hc(taxis,faxis,STRF,Wo,PP,Sound,MaxFm,MaxRD,Thresh,alpha1,alpha2,Disp,chStr)

%Input Arguments
if nargin<6 |isempty(Sound)
    Sound='MR';
end
if nargin<7 | isempty(MaxFm)
    MaxFm=500;
end
if nargin<8 | isempty(MaxRD)
    MaxRD=4;
end
if nargin<9  | isempty(Thresh)
    Thresh=0.5;   %Half Power Threshold 
end
if nargin<10 | isempty(alpha1)
    alpha1=0.05;
end
if nargin<11 | isempty(alpha2)
    alpha2=0.1;
end
if nargin<12 | isempty(Disp)
    Disp='n';
end
...
taxis=1000*taxis;                   %MAE, Oct 2015; milliseconds
...
RFParam.PeakBF=X(i);
[i,j]=find(max(max(STRF))==STRF);
RFParam.PeakDelayP=taxis(j);
RFParam.PeakBFP=X(i);
[i,j]=find(min(min(STRF))==STRF);
RFParam.PeakDelayN=taxis(j);
% disp(RFParam.PeakDelayP)
RFParam.PeakBFN=X(i);
...
%Finding Duration at 50% of Max Envelope
...
%Finding Envelope Bandwidth at 50% of peak envelope
...
% RFParam.HalfEnvBW=X2-X1; % octave
RFParam.HalfEnvBW=faxis(1)*(2^X2-2^X1); % Hz
RFParam.X1=X1;
RFParam.X2=X2;
RFParam.Pf=Pf;
...
% RFParam.EnvBW10=XU10-XL10; % octave
RFParam.EnvBW10=faxis(1)*(2^XU10-2^XL10); % Hz
RFParam.XL10=XL10;
RFParam.XU10=XU10;
